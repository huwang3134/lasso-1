...
variable = c(trace1$coef[,2] %>% as.character(), 
             ...
             trace6$coef[,2] %>% as.character()) %>% unique() %>% as.data.frame()
...
MSE = c(variable = "MSE" , 
           "Lasso 1" = mse.1 , "Lasso 2" = mse.2 , 
           "Lasso 3" = mse.3 , "Lasso 4" = mse.4 , 
           "Lasso 5" = mse.5 , "Lasso 6" = mse.6)
MSE[-1] = as.numeric(MSE[-1]) %>% round(digits = 6)
sum.table = rbind(Trans, VECM, Maxlag, MSE, Lambda, sum.table)
